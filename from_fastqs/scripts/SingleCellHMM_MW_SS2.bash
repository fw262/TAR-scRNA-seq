...
refFlat=$2 #gene annotations in refFlat format
CORE=$3
MERGEBP=$4
THRESH=$5
PL=$6
...
bedtools bamtobed -i ${INPUT_BAM} -split -bedpe |cut -f1,2,6,7,8,9 |LC_ALL=C sort -k1,1V -k2,2n --parallel=30| awk '{print $0}' | gzip > ${TMPDIR}/${PREFIX}_split.sorted.bed.gz
...
Rscript scripts/generate_refFlat_script_both.R ${refFlat} TAR_reads.bed.gz
Rscript scripts/convertRefFlatToGTF.R TAR_reads.bed.gz.noDir.refFlat.refFlat
